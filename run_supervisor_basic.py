...
Basic supervisor run script for MLGym.
...
from mlgym.environment.supervisor_env import SupervisorEnv
...
class SupervisorScriptArguments(FlattenedAccess, FrozenSerializable):
    ...
    def run_name(self) -> str:
        ...
        # Format: supervisor_basic__{task}__{model}__{timestamp}
        base_name = f"supervisor_basic__{task_id}__{model_name}__{timestamp}"
...
class SupervisorMain:
    ...
    def run(self, supervisor: SupervisorAgent, env: SupervisorEnv) -> None:
        ...
        add_file_handler(log_path, [
            ...
            "env_utils", "MLGymEnv", "supervisor_env"
        ])
        ...
        # Set default agent args for worker agents
        env.default_agent_args = AgentArguments(
            model=ModelArguments(model_name="litellm:gpt-4o-mini"),
            agent_config_path=CONFIG_DIR / "agents" / "worker_template.yaml",
        ...
        while not done and step < self.args.max_supervisor_steps:
            step += 1
            logger.info(f"🔄 Supervisor step {step}")
            ...
            state_data = {
                ...
                "current_step": step - 1,
                "remaining_steps": self.args.max_supervisor_steps - step + 1
            }
            import json
            ...
            if info.get("dont_count_step", False):
                step -= 1  # Don't count this step
                logger.info(f"🔄 Parsing error - not counting step {step + 1}")
            else:
        ...
        for i, agent_name in enumerate(env.completed_agents):
            steps = env.agent_steps.get(agent_name, 0)
            logger.info(f"   Agent {i+1}: {agent_name} - {steps} steps completed")
    ...
    async def run_supervisor(self) -> None:
        ...
        # Create supervisor environment directly (not through gym.make)
        env = SupervisorEnv(self.args.environment, devices=[], render_mode=None, supervisor_args=self.args)
